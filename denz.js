...
try {
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© F O X Z Y B O T Z_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             denz.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `${NamaBot}`, "footerText": "*_© F O X Z Y B O T Z_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
const daftar1 = `Hai kak  ${pushname} ${tampilUcapan} \n\nSebelum Mengakses Bot Verify Terlebih Dahulu Ya Anj Gunain Bot Dengan Bijak `
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© F O X Z Y B O T Z_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"DON'T CLICK HERE",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/508555151/508555151`}}})
		}
		const simir = (teks) => {
...
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_© Foxzy@IforceSec_*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
reply('http://instagram.com/luxzy_store1')
}
if(isButton == 'ytm'){
reply('http://youtube.com/c/FoxzyModder')
}
...
"thumbnail": pporigi,
       ...
       menu = `┌───「 \`\`\`${NamaBot}\`\`\` 」
│
├ _Creator : *Foxzy@IforceSec* (CUMAN RECODE)
...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© F O X Z Y B O T Z_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
stod = `${sender}`
       stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🦈 F O X Z Y B O T Z 🦈'
       ownerJid = "508555151@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `╭─❒ 「 Bot Info 」 ❒
├ _Creator : *Foxzy@IforceSec* (CUMAN RECODE)
...
case 'command':
 ...
 footerText: '*_© F O X Z Y B O T Z_*',
...
case 'maker3': case 'maker4':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
if (args.length < 1) return reply(`*Example :*\n${prefix + command} Foxzy@IforceSec`)
...
const skuygelud = {
    contentText: `${anu}`,
    footerText: '*_©FoxzyXNodeJS BOTZ_*',
    buttons: buttons,
    headerType: 1
}
...
client.browserDescription = ['Foxzy@IforceSec','Desktop','3.0']
...
break
		...
		case 'sc':
		case 'sourcecode':
		anu =`*FoxzyXNodeJS BOTZ SEWABOT*
		15K PERMANEN BWANG:) RUN HEROKU 24JAM
		MINAT PC OWNER SAJA`
 buttons = [
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://github.com/foxzyganzz"
        },
        "index": 0
      }
    ]
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              denz.groupUpdateSubject(from, `HACKED BY F O X Z Y B O T Z`)
...
Giliran = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer F O X Z Y B O T Z",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/p/508555151/508555151"}}})
	}
    }
    }
    }
